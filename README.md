...
- 使用 Chrome Side Panel API，支持日间 / 夜间主题切换
...
压缩包输出到 `.output/`，文件名形如 `free-novel-1.0.2-chrome.zip`（版本号取自 `package.json`）。

## 在 Chrome 中加载

1. 打开 `chrome://extensions/`
2. 开启右上角“开发者模式”
3. 将 `.output/` 下打包生成的 `free-novel-1.0.2-chrome.zip` 直接拖入扩展程序页面
4. 点击插件图标打开侧边栏
...
书源网站的页面结构、访问限制和可用性可能随时变化。部分书源可能搜索失败或无法完整读取，搜索页会列出失败的书源及具体原因；下载记录中也会保存失败状态。
